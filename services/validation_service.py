...
class UsuarioValidationService:
    ...
    def validar(
        ...
        ignorar_id:
            Usado na edição para não considerar o próprio usuário
            como duplicado.
        """

        dados_normalizados = self._normalizar_dados(dados)

        sucesso, mensagem = self._validar_campos_obrigatorios(
            dados_normalizados
        )
        ...
        return True, "Dados válidos.", dados_normalizados

    def _normalizar_dados(self, dados: dict) -> dict:
        return {
            "nome": str(dados.get("nome") or "").strip(),
            "cpf": CpfService.formatar(dados.get("cpf") or ""),
            "email": EmailService.normalizar(dados.get("email") or ""),
    ...
    def _validar_campos_obrigatorios(
        ...
        for campo, nome_amigavel in self.CAMPOS_OBRIGATORIOS.items():
            if not campo_obrigatorio(dados.get(campo)):
                return False, f"O campo '{nome_amigavel}' é obrigatório."
    ...
    def _validar_email(
        ...
        email_valido, mensagem = EmailService.validar(email)
    ...
    def _validar_celular(
        ...
        if len(digitos) not in (10, 11):
            return False, "Informe um celular válido com DDD."
        ...
        if len(numero) == 9 and not numero.startswith("9"):
            return False, "Celular com 9 dígitos deve começar com 9."

        if len(numero) == 8 and numero[0] not in ("2", "3", "4", "5"):
